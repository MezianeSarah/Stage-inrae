...
def peridynamic_output3(points, bounds, output_file="output.txt"):
    vor = Voronoi(points)
    ...
    with open(output_file, 'w') as f:
        ...
        for ridge in vor.ridge_vertices:
            ...
            if (xmin <= v0[0] <= xmax and ymin <= v0[1] <= ymax and
                ...
                for x3, y3, x4, y4 in [[xmin,ymin,xmax,ymin],[xmax,ymin,xmax,ymax],[xmin,ymax,xmax,ymax],[xmin,ymin,xmin,ymax]]:
                    intersect, point = segments_intersect(v0[0], v0[1], v1[0], v1[1], x3, y3, x4, y4)
                    if not intersect:
                        continue
                    if point_outside_box(v0[0], v0[1], v1[0], v1[1], xmin, ymin, xmax, ymax) == (v0[0], v0[1]):
                        v0 = np.array(point)
                    elif point_outside_box(v0[0], v0[1], v1[0], v1[1], xmin, ymin, xmax, ymax) == (v1[0], v1[1]):
                        v1 = np.array(point)
                    if (xmin <= v0[0] <= xmax and ymin <= v0[1] <= ymax and
                        xmin <= v1[0] <= xmax and ymin <= v1[1] <= ymax):
                        f.write(f"{v0[0]}\t{v0[1]}\t! coordonnées du point 1 du segment {compt}\n")
...
for model_id in range(1, num_models + 1):
    ...
    points = []
    dx = np.random.uniform(0.7, 1.3) * scale_factor
    dy = np.sqrt(3) / 2 * dx
    x_vals = np.arange(x_min, x_max, dx)
    y_vals = np.arange(y_min, y_max, dy)

    for i, y in enumerate(y_vals):
        offset = dx / 2 if i % 2 else 0
        for x in x_vals:
            jitter_x = np.random.uniform(-0.05 * dx, 0.05 * dx)
            jitter_y = np.random.uniform(-0.05 * dy, 0.05 * dy)
            px = x + offset + jitter_x
            py = y + jitter_y
            points.append([px, py])
    ...
    for i, region in enumerate(regions):
        point = vor.points[i]
        is_top = np.abs(point[1] - max_y) < 0.3
        is_bottom = np.abs(point[1] - min_y) < 0.3
...
print(f"✅ {num_models} modèles générés avec VARIÉTÉ hexagonale dans : '{output_dir}'")
